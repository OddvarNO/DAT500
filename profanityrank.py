...
# Want to get a rank of how profane a website is by the amount of profane words.
...
class MRProfanityrank(MRJob):
    ...
    def mapper_profRank(self, _, line):
        ...
        if line.find("parsed_text: ") == 0:
            ...
            #     self.profanityRank = (len(pr) / len(tot_word)) / (len(set(pr)) / len(PROFANE_WORDS))
            #     if self.profanityRank > 1: # Set to one and not zero to remove results from pages that contain too few profane words, or simply another language.
            #         self.allLinks[self.url] = self.links
            #         self.allprofanityRanks[self.url] = self.profanityRank

            if self.endUrl == "https://zzsh111.com/a/Hu4SGefM/":

                yield self.allLinks, self.allprofanityRanks
            # self.url = ""
            # self.profanityRank = 0
    ...
    def mapper_createMatrix(self, urls, ranks):
        ...
        yield ranks, (v.tolist(), lengths)    


    # SHOULD USE TF-IDF for profanityrank 
    # Formula for profanityrank: PrR(u) = un_pr/totUn_pr + pr/tot_word (Perhaps a connection to how profane sites that this site links to are)
        # Highest rank = 0.15523656107413283 or 0.2155219309894625 
    # Alternative formula for profanityrank: PrR(u) =  (pr/tot_word) / (un_pr/totUn_pr) (AND a connection to how profane sites that this site links to are)
        # Highest rank = 13.192982456140351 or 36.054794520547944
    # Timing with stopwords = 3m39.595s / From a simple early implementation
    # Timing without stopwords = 3m16.207s / From a simple early implementation
